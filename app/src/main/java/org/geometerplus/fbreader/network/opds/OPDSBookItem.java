...
public class OPDSBookItem extends NetworkBookItem implements OPDSConstants {
    ...
    public static OPDSBookItem create(final NetworkLibrary library, ZLNetworkContext nc, INetworkLink link, String url)
    ...
    private static void collectReferences(NetworkLibrary library, UrlInfoCollection<UrlInfo> urls, OPDSLink opdsLink, String href, UrlInfo.Type type, Money price, boolean addWithoutFormat) {
        boolean added = false;
    ...
    public OPDSBookItem(NetworkLibrary library, OPDSNetworkLink link, String id, int index, CharSequence title, CharSequence summary, List<AuthorData> authors, List<String> tags, String seriesTitle, float indexInSeries, UrlInfoCollection<?> urls) {
    ...
    OPDSBookItem(NetworkLibrary library, OPDSNetworkLink networkLink, OPDSEntry entry, String baseUrl, int index) {
        this(library, networkLink, entry.Id.Uri, index, entry.Title, getAnnotation(entry), getAuthors(entry), getTags(entry), entry.SeriesTitle, entry.SeriesIndex, getUrls(library, networkLink, entry, baseUrl));
    }
