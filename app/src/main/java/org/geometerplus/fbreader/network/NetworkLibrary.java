...
public class NetworkLibrary {
    ...
    private final List<INetworkLink> myLinks = Collections.synchronizedList(new ArrayList<INetworkLink>());
    private final Set<ChangeListener> myListeners = Collections.synchronizedSet(new HashSet<ChangeListener>());
    private final Map<NetworkTree, NetworkItemsLoader> myLoaders = Collections.synchronizedMap(new HashMap<NetworkTree, NetworkItemsLoader>());

    private final Map<String, WeakReference<ZLImage>> myImageMap = Collections.synchronizedMap(new HashMap<String, WeakReference<ZLImage>>());
    ...
    private ZLStringListOption activeIdsOption() {
        if (myActiveIdsOption == null) {
            myActiveIdsOption = new ZLStringListOption("Options", "ActiveIds", "", ",");
        }
    ...
    public boolean isLoadingInProgress(NetworkTree tree) {
        return (tree instanceof NetworkCatalogTree && ((NetworkCatalogTree)tree).Item.UpdatingInProgress) || getStoredLoader(tree) != null;
    }
