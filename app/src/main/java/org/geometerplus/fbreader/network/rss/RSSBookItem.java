...
public class RSSBookItem extends NetworkBookItem {
    ...
    public RSSBookItem(RSSNetworkLink link, String id, int index, CharSequence title, CharSequence summary, List<AuthorData> authors, List<String> tags, String seriesTitle, float indexInSeries, UrlInfoCollection<?> urls) {
    ...
    RSSBookItem(RSSNetworkLink networkLink, RSSItem entry, String baseUrl, int index) {
        this(networkLink, entry.Id.Uri, index, entry.Title, getAnnotation(entry), getAuthors(entry), getTags(entry), null, 0, null);
    }
